...
import { createCheckout, createPortal, getUltra, handleStripeWebhook, setPreviewUltra } from "../services/billing";
...
export const billingRoutes: RouteDefinition[] = [
  ...
  { method: "POST", path: "/v1/ultra/preview", auth: true, handler: async context => {
    const body = await parseJson<{ enabled: boolean; cadence?: "monthly" | "annual" }>(context.request);
    return json(await setPreviewUltra(context, body.enabled, body.cadence ?? "monthly"));
  } },
