...
export type GroupTrigger = "user" | "continue" | "typing" | "quiet" | "away";
...
export function groupCharacterContext(character: GroupCharacter, characters: GroupCharacter[], transcript: GroupTranscriptMessage[], userIdentity = "The user", contextBudget = 4_000) {
  const roster = characters.map(member => ({id: member.id, name: member.name, description: member.tagline}));
  const recent: GroupTranscriptMessage[] = [];
  let remaining = contextBudget;
  for (const message of transcript.slice(-50).reverse()) {
